...
export default function TestChatPage() {
  ...
  if (!storeId) {
    return <div className="max-w-2xl mx-auto text-paper-700/60">Cargando...</div>;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col h-full">
      ...
      <div className="flex-1 bg-white rounded-xl border border-paper-200 overflow-hidden flex flex-col shadow-sm">
        <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[400px] max-h-[60vh]">
          {messages.length === 0 && (
            <p className="text-center text-paper-700/50 mt-10">
            ...
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm ${
                msg.role === "user"
                  ...
                  : "bg-paper-50 border border-paper-100 text-paper-950 rounded-bl-md"
              }`}>
                <span className="whitespace-pre-wrap break-words">{msg.text}</span>
                {msg.meta && (
                  <div className="mt-2 pt-2 border-t border-paper-200 text-xs text-paper-700/70 space-y-0.5">
                    ...
                    {Array.isArray(msg.meta.parsed_items) && (
                      <details>
                        ...
                        <pre className="mt-1 p-1 bg-paper-100/50 rounded text-xs overflow-x-auto">
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-paper-50 border border-paper-100 px-4 py-2.5 rounded-2xl rounded-bl-md text-sm text-paper-700/70">
                Escribiendo...
              </div>
            </div>
          )}
        ...
        <form
          ...
          className="border-t border-paper-100 p-3 flex gap-2 items-end"
        >
          <div className="flex-1 flex flex-col gap-2">
            {receiptPreview && (
              <div className="flex items-center gap-2 bg-paper-50 rounded-lg p-2">
                <img src={receiptPreview} alt="Preview" className="h-12 w-12 object-cover rounded" />
                <span className="text-xs text-paper-700/70 truncate flex-1">{receiptFile?.name}</span>
                ...
                <button type="button" onClick={handleSendReceipt} disabled={sendingReceipt}
                  className="px-3 py-1 text-xs font-medium text-paper-950 bg-signal-400 rounded-lg hover:bg-signal-500 disabled:opacity-50">
            ...
            <div className="flex gap-2">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
                ...
                className="flex-1 px-3 py-2 text-sm border border-paper-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-paper-200 focus:border-signal-400 bg-white"
                disabled={loading} />
              <label className="px-3 py-2 text-sm border border-paper-200 rounded-lg cursor-pointer hover:bg-paper-50 text-paper-700/70">
                📎
